import { Webhook, Terminal, GitBranch } from 'lucide-react';

export default function Sidebar() {
    const onDragStart = (event: React.DragEvent, nodeType: string) => {
        // Store the node type so the canvas knows what was dropped
    ...
    return (
        <aside className="w-64 border-r border-border bg-card p-4 flex flex-col gap-4 shadow-sm z-10">
            ...
            <div
                className="p-3 border rounded-md cursor-grab active:cursor-grabbing bg-muted border-border flex items-center gap-3 transition-colors hover:bg-accent"
                onDragStart={(e) => onDragStart(e, 'log')}
                draggable
            >
